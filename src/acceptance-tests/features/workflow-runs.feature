Feature: Workflow run and job data retrieval
  ...
  Scenario: All workflow definitions are listed for a repository
    Given a repository with workflow definitions available from the GitHub API
    When the exporter lists workflow definitions for the repository
    Then all workflow definitions are returned
  ...
  Scenario: GitHub API returns an error for one run's jobs endpoint
    Given a repository with 2 workflows where the first run has a failing jobs endpoint
